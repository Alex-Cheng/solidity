...
void SyntaxTest::printExpected(ostream& _stream, string const& _linePrefix, bool const _formatted) const
{
	if (m_expectations.empty())
		FormattedScope(_stream, _formatted, {GREEN}) << _linePrefix << "Success" << endl;
	else
		for (auto const& expectation: m_expectations)
			FormattedScope(_stream, _formatted, {expectation.type == "Warning" ? YELLOW : RED}) <<
...
) const
{
	if (_errorList.empty())
		...
		for (auto const& error: _errorList)
		{
			...
			FormattedScope scope(_stream, _formatted, {isWarning ? YELLOW : RED});
			_stream << _linePrefix;
			if (_lineNumbers)
			{
