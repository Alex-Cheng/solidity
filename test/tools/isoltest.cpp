...
void SyntaxTestTool::printContract() const
{
	...
	string line;
	FormattedScope scope(cout, m_formatted, {CYAN});
...
SyntaxTestTool::Result SyntaxTestTool::process()
{
	...
	{
		FormattedScope(cout, m_formatted, {RED}) << "cannot read test: " << _e.what() << endl;
	...
	if (success)
	{
		FormattedScope(cout, m_formatted, {GREEN}) << "OK" << endl;
		return Result::Success;
	}
	else
	{
		FormattedScope(cout, m_formatted, {RED}) << "FAIL" << endl;
...
Allowed options)",
	...
	if (fs::exists(syntaxTestPath) && fs::is_directory(syntaxTestPath))
	{
		...
		cout << endl << "Summary: ";
		FormattedScope(cout, formatted, {stats ? GREEN : RED}) <<
